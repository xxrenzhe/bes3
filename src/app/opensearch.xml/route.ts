...
export async function GET(request: Request) {
  ...
  const body = [
    ...
    `<Description>${escapeXml('Search Bes3 products, reviews, comparisons, guides, and deals.')}</Description>`,
    ...
    `<Tags>${escapeXml('search reviews comparisons guides deals products')}</Tags>`,
