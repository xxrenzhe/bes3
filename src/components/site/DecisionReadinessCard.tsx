  ...
  return (
    <div className="rounded-md border border-border bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">Store-link check</p>
      ...
      <ul className="mt-3 space-y-2 text-xs leading-6 text-muted-foreground">
        {readiness.reasons.slice(0, 2).map((reason) => <li key={reason}>Good sign: {reason}</li>)}
        {readiness.blockers.slice(0, 2).map((blocker) => <li key={blocker}>Still missing: {blocker}</li>)}
      </ul>
    </div>
  )
}
