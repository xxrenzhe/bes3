...
export const GEMINI_SUPPORTED_MODELS = OFFICIAL_SUPPORTED_MODELS
...
export function getSupportedModelsForProvider(provider?: string | null): readonly AIModel[] {
  return provider === 'relay' ? RELAY_SUPPORTED_MODELS : OFFICIAL_SUPPORTED_MODELS
}
...
export function isModelSupportedByProvider(model?: string | null, provider?: string | null): boolean {
  ...
  return provider === 'relay' ? RELAY_MODEL_SET.has(model) : OFFICIAL_MODEL_SET.has(model)
}
