...
const DEFAULT_SETTINGS = [
  ['ai', 'gemini_provider', (process.env.AI_PROVIDER || process.env.GEMINI_PROVIDER || 'official') === 'gemini' ? 'official' : (process.env.AI_PROVIDER || process.env.GEMINI_PROVIDER || 'official'), 'string', 0, 'Gemini provider'],
  ['ai', 'gemini_model', GEMINI_ACTIVE_MODEL, 'string', 0, 'Gemini model'],
  ['ai', 'gemini_endpoint', process.env.GEMINI_BASE_URL || 'https://generativelanguage.googleapis.com', 'string', 0, 'Gemini API endpoint'],
  ['ai', 'gemini_api_key', '', 'secret', 1, 'Gemini API key used for keyword and article generation'],
  ['ai', 'gemini_relay_api_key', '', 'secret', 1, 'Relay API key used when AI provider is relay'],
  ['ai', 'geminiTimeoutMs', '30000', 'string', 0, 'Gemini request timeout in milliseconds'],
  ['proxy', 'urls', '[]', 'json', 0, 'Proxy pool list'],
  ...
  ['deepScrape', 'enabled', process.env.DEEP_PRODUCT_SCRAPE_ENABLED || 'true', 'boolean', 0, 'Enable Playwright-based deep browser product scraping before normalization'],
  ['deepScrape', 'timeoutMs', process.env.DEEP_PRODUCT_SCRAPE_TIMEOUT_MS || '60000', 'number', 0, 'Deep browser scrape navigation timeout in milliseconds'],
  ['deepScrape', 'waitAfterLoadMs', process.env.DEEP_PRODUCT_SCRAPE_WAIT_AFTER_LOAD_MS || '1500', 'number', 0, 'Extra wait after network settle before extracting browser signals'],
  ['deepScrape', 'maxAttempts', process.env.DEEP_PRODUCT_SCRAPE_MAX_ATTEMPTS || '2', 'number', 0, 'Maximum deep scrape attempts recorded per task'],
  ['deepScrape', 'requireProxy', process.env.DEEP_PRODUCT_SCRAPE_REQUIRE_PROXY || 'false', 'boolean', 0, 'Require a configured proxy before deep product scraping runs'],
  ['affiliate_sync', 'partnerboost_base_url', 'https://app.partnerboost.com', 'string', 0, 'PartnerBoost API base URL'],
  ['affiliate_sync', 'partnerboost_token', '', 'secret', 1, 'PartnerBoost token'],
  ['affiliate_sync', 'amazonPageSize', process.env.PARTNERBOOST_AMAZON_PAGE_SIZE || '20', 'number', 0, 'PartnerBoost Amazon products fetched per page'],
  ['affiliate_sync', 'dtcPageSize', process.env.PARTNERBOOST_DTC_PAGE_SIZE || '20', 'number', 0, 'PartnerBoost DTC products fetched per page'],
  ['affiliate_sync', 'maxPagesPerSync', process.env.PARTNERBOOST_MAX_PAGES_PER_SYNC || '5', 'number', 0, 'Maximum PartnerBoost pages fetched in one admin sync'],
